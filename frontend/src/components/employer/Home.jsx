...
import { FaBars, FaPlus, FaClipboardList, FaUser, FaComments, FaRegStar, FaFileAlt } from "react-icons/fa"; // Updated icons
  ...
  return (
    <div className="min-h-screen bg-green-100 flex flex-col items-center overflow-x-hidden">
      ...
      <div className="w-full max-w-screen-sm px-4 pt-4">
        <div className="bg-white p-4 rounded-xl shadow-md">
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: "My Posts", icon: <FaClipboardList /> }, // Changed icon for My Posts
              { label: "New Job", icon: <FaPlus /> },
              { label: "Applications", icon: <FaFileAlt /> },
              { label: "Profile", icon: <FaUser /> },
              { label: "Chat", icon: <FaComments /> },
              { label: "Subscriptions", icon: <FaRegStar /> }, // Changed icon for Subscriptions
            ].map(({ label, icon }) => (
              <button
                key={label}
      ...
      <div className="w-full max-w-screen-sm px-4 pt-6 pb-8 grid gap-4">
        {jobData.map((job) => (
          <div key={job.id} className="bg-white rounded-lg shadow-md p-4 flex gap-6 items-center">
            <img src={job.image} alt={job.title} className="w-20 h-20 object-contain" />
